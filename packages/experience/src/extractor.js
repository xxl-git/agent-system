...
exports.getExperienceExtractor = getExperienceExtractor;
...
class ExperienceExtractor {
     ...
     */
    async extract(userInput, agentReply, outcome, sessionId, context) {
        // 尝试 LLM 提取
        ...
        if (!extracted || extracted.scenario === 'N/A') {
            logger_1.default.debug('[ExperienceExtractor] 无有价值的经验可提取');
            return null;
        }
        // 确定经验类型
        const type = extracted.type || (outcome === 'success' ? 'pattern' : 'pitfall');
        // 构造 ExperienceInput
        const input = {
        ...
        const store = (0, store_1.getExperienceStore)();
     ...
     * 用户手动保存经验
     */
    async manualSave(scenario, problem, solution, opts) {
        const input = {
            scenario,
            problem,
            solution,
    ...
    async refine(rawText, existing) {
        ...
        // 检索相似经验（用于去重提示）
        const retriever = (0, retriever_1.getExperienceRetriever)();
        const similar = retriever.retrieve(`${draft.scenario} ${draft.problem} ${draft.solution}`, { topK: 3 });
        return { draft, similar };
    }
    // ─── LLM 精炼 ──────────────────────────────────────────────────────────────
    async llmRefine(rawText, existing) {
        try {
            const registry = (0, registry_1.getPromptRegistry)();
            const tpl = registry.get('experience.refine');
            const parts = [`用户描述: ${rawText}`];
            if (existing) {
            ...
            const router = (0, llm_router_1.getLLMRouter)();
            ...
            if (!jsonMatch) {
                logger_1.default.warn('[ExperienceExtractor] refine: LLM 返回无 JSON');
                return null;
            }
            const parsed = JSON.parse(jsonMatch[0]);
            if (!parsed.scenario || !parsed.problem || !parsed.solution) {
                logger_1.default.warn('[ExperienceExtractor] refine: LLM 结果不完整');
                return null;
            }
            return {
            ...
            logger_1.default.warn('[ExperienceExtractor] refine: LLM 失败，降级规则引擎', err);
            return null;
        }
    }
    // ─── 规则引擎精炼兜底 ──────────────────────────────────────────────────────
    ruleEngineRefine(rawText, existing) {
        if (existing) {
            // 编辑模式：保留原有结构，只更新内容
            return {
    ...
    async llmExtract(userInput, agentReply, outcome, errors) {
        try {
            const registry = (0, registry_1.getPromptRegistry)();
            ...
            const jsonMatch = content.match(/\{[\s\S]*\}/);
            if (!jsonMatch) {
                logger_1.default.warn('[ExperienceExtractor] LLM 返回无 JSON');
                return null;
            }
            const parsed = JSON.parse(jsonMatch[0]);
            // 基本校验
            if (!parsed.scenario || !parsed.problem || !parsed.solution) {
                logger_1.default.warn('[ExperienceExtractor] LLM 提取结果不完整');
                return null;
            }
            return {
                scenario: String(parsed.scenario),
                ...
                reasoning: String(parsed.reasoning || ''),
                tags: Array.isArray(parsed.tags) ? parsed.tags.map(String) : [],
                type: parsed.type || (outcome === 'success' ? 'pattern' : 'pitfall'),
            };
        }
        catch (err) {
            logger_1.default.warn('[ExperienceExtractor] LLM 提取失败，降级规则引擎', err);
            return null;
        }
    }
    // ─── 规则引擎兜底 ──────────────────────────────────────────────────────────
    ruleEngineExtract(userInput, agentReply, outcome, errors) {
        // 如果回复太短或无实质内容，跳过
        if (agentReply.length < 20)
            return null;
    ...
    guessScenario(userInput) {
        const lower = userInput.toLowerCase();
        if (lower.includes('编译') || lower.includes('compile') || lower.includes('build'))
            return '代码编译';
        if (lower.includes('测试') || lower.includes('test'))
            return '运行测试';
        if (lower.includes('文件') || lower.includes('file') || lower.includes('读写'))
            return '文件操作';
        if (lower.includes('配置') || lower.includes('config'))
            return '配置管理';
        if (lower.includes('模型') || lower.includes('model') || lower.includes('llm'))
            return '模型调用';
...
// ─── 单例 ────────────────────────────────────────────────────────────────────
let _instance = null;
function getExperienceExtractor() {
    if (!_instance) {
