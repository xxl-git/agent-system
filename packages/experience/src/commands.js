...
exports.ExperienceCommandHandler = void 0;
exports.getExperienceCommandHandler = getExperienceCommandHandler;
...
class ExperienceCommandHandler {
    ...
    async handleAdd(rawText) {
        ...
        const extractor = (0, extractor_1.getExperienceExtractor)();
    ...
    async handleEdit(idStr) {
        ...
        const extractor = (0, extractor_1.getExperienceExtractor)();
        const { draft, similar } = await extractor.refine(`编辑经验 #${id}: ${rec.scenario} — ${rec.problem} — ${rec.solution}`, {
    ...
    handleSearch(keyword) {
        ...
        const lines = [`搜索 "${keyword}" — 找到 ${results.length} 条:`];
    ...
    // ─── 格式化 ────────────────────────────────────────────────────────────────
    formatDraft(draft, similar, isEdit, editId) {
    ...
    formatRecord(rec) {
        ...
        if (rec.project)
            lines.push(`   项目: ${rec.project}`);
        if (rec.modelUsed)
            lines.push(`   模型: ${rec.modelUsed}`);
        lines.push('');
        lines.push('/exp edit ' + rec.id + ' 编辑 | /exp delete ' + rec.id + ' 删除');
        return lines.join('\n');
    }
}
exports.ExperienceCommandHandler = ExperienceCommandHandler;
// ─── 单例 ────────────────────────────────────────────────────────────────────
let _instance = null;
function getExperienceCommandHandler() {
    if (!_instance) {
