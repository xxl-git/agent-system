...
import { ChatMessage, LLMClient, PromptRegistry, DefaultLLMClient, DefaultPromptRegistry } from './llm-client';
...
export class ExperienceExtractor {
  ...
  constructor(
    ...
    this.promptRegistry = promptRegistry || new DefaultPromptRegistry();
  }
  /**
   * 从任务执行中提取经验
   *
  ...
  private async llmRefine(
    ...
    try {
      const tpl = this.promptRegistry.get('experience.refine');
      ...
      const content = await this.llmClient.call(messages);
  ...
  private async llmExtract(
    ...
    try {
      const tpl = this.promptRegistry.get('experience.extract');
