...
// Phase 5 P1: 解决重启失忆
import { getDBStore, type DecisionRecord, type EntityRecord, type SummaryRecord, type SessionRecord } from './db-store';
import { getMemoryStore, FileMemoryStore } from './file-store';
...
export class SessionRecoverer {
  ...
  recover(): MemoryInjection {
    ...
    try {
      ...
      if (stats.includes('decisions') && !stats.includes('decisions: 0')) {
        // SQLite 不支持完整搜索，换个方式：加载所有决策再截取最近N条
        recentDecisions = this.loadRecentDecisions(db);
      }
      if (stats.includes('entities') && !stats.includes('entities: 0')) {
        trackedEntities = this.loadTopEntities(db);
      }
      if (stats.includes('summaries') && !stats.includes('summaries: 0')) {
  ...
  private loadRecentFileMemory(memStore: any): string {
    ...
    try {
      ...
      if (todayContent) {
        ...
        parts.push(`  📅 ${date}.md: ${brief}${todayContent.length > 500 ? '...' : ''}`);
      }
      ...
      for (const file of files) {
        ...
        if (now - stat.mtimeMs > this.config.recentDays * 86400000) continue;

        const content = readFileSync(filePath, 'utf-8');
        const brief = content.slice(0, 500).replace(/\n{3,}/g, '\n\n');
        parts.push(`  📅 ${file}: ${brief}${content.length > 500 ? '...' : ''}`);
      }
    } catch (err) {
  ...
  private buildPromptBlock(
    ...
    if (entities.length > 0 && charCount < limit) {
      ...
      for (const e of entities) {
        const line = `  ${e.type}: ${e.name} (提及 ${e.mention_count} 次)${e.notes ? ' - ' + e.notes : ''}`;
        if (charCount + line.length > limit) break;
        lines.push(line);
        charCount += line.length + 1;
      }
