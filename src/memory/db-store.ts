...
export class DBStore {
  ...
  // === 导出/统计 ===
  /** 返回结构化统计（供 Dashboard 使用） */
  getStatsObject(): { decisions: number; entities: number; sessions: number; summaries: number } {
    const result = { decisions: 0, entities: 0, sessions: 0, summaries: 0 };
    const tables: Array<keyof typeof result> = ['decisions', 'entities', 'sessions', 'summaries'];
    for (const table of tables) {
      try {
        const rows = this.db.exec(`SELECT COUNT(*) as cnt FROM ${table}`);
        if (rows.length > 0 && rows[0].values.length > 0) {
          result[table] = Number(rows[0].values[0][0]);
        }
  ...
  getStats(): string {
    const obj = this.getStatsObject();
    return `decisions: ${obj.decisions} 条\nentities: ${obj.entities} 条\nsessions: ${obj.sessions} 条\nsummaries: ${obj.summaries} 条`;
  }
