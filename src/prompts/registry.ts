...
import type { LLMParams } from '../llm/llm-router';
...
export class PromptRegistry {
  ...
  register(template: PromptTemplate): void {
    ...
    logger.debug(`[PromptRegistry] 注册模板: ${template.id} v${template.version}`);
  }
  ...
  get(id: string, variables?: Record<string, string>): PromptTemplate {
    ...
    if (!tpl) {
      logger.warn(`[PromptRegistry] 模板不存在: ${id}，返回空占位`);
  ...
  loadFromDir(promptsDir: string): void {
    if (!fs.existsSync(promptsDir)) {
      logger.warn(`[PromptRegistry] 提示词目录不存在: ${promptsDir}`);
      return;
    }
    ...
    for (const file of files) {
      try {
        ...
        logger.warn(`[PromptRegistry] 加载模板文件失败: ${file}`, err);
      }
    }
    logger.info(`[PromptRegistry] 从目录加载了 ${loaded} 个模板: ${promptsDir}`);
  }
...
只输出 JSON。`,
    ...
    logger.debug(`[PromptRegistry] 内置模板注册完成，共 ${this.templates.size} 个`);
  }
}
...
function interpolate(template: string, variables: Record<string, string>): string {
  return template.replace(/\{\{(\w+)\}\}/g, (_, key) => {
    ...
    logger.warn(`[PromptRegistry] 插值变量未提供: {{${key}}}`);
    return `{{${key}}}`;
  });
}
 ...
 *   系统提示词内容
 */
function parseTemplateFile(id: string, raw: string): PromptTemplate {
  ...
  if (versionMatch) fm.version = versionMatch[1].trim();
  const wrapperMatch = frontmatterStr.match(/wrapper:\s*["']?(minimal|structured|verbose)["']?/);
  if (wrapperMatch) fm.wrapper = wrapperMatch[1] as 'minimal' | 'structured' | 'verbose';
  const taskTypeMatch = frontmatterStr.match(/taskType:\s*["']?(\w+)["']?/);
  if (taskTypeMatch) fm.taskType = taskTypeMatch[1] as LLMTaskType;
  ...
  if (paramsMatch) {
    ...
    if (tokensMatch) params.max_tokens = parseInt(tokensMatch[1], 10);
    const reasoningMatch = paramsStr.match(/reasoning:\s*["']?(\w+)["']?/);
    if (reasoningMatch) params.reasoning = reasoningMatch[1];
    fm.params = params;
  }
...
let _instance: PromptRegistry | null = null;
...
export function initPromptRegistry(promptsDir?: string): PromptRegistry {
  _instance = new PromptRegistry();
  if (promptsDir) {
