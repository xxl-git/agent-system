...
export class ExperienceRetriever {
  private store = getExperienceStore();
  ...
  retrieve(userInput: string, opts?: RetrieveOptions): RetrieveResult[] {
    ...
    if (top.length > 0) {
      logger.debug(
        `[ExperienceRetriever] "${userInput.slice(0, 50)}" → 匹配 ${results.length} 条，取 Top-${top.length}` +
  ...
  private matchScore(userInput: string, rec: ExperienceRecord): { score: number; reason: string } {
  ...
  private tagMatch(input: string, tags: string[]): number {
    if (tags.length === 0) return 0;
    let hits = 0;
    for (const tag of tags) {
      if (input.includes(tag.toLowerCase())) hits++;
    }
    return hits === 0 ? 0 : 0.6 + (hits / tags.length) * 0.3; // 0.6 - 0.9
  }

  private keywordMatch(input: string, rec: ExperienceRecord): number {
...
let _instance: ExperienceRetriever | null = null;

export function getExperienceRetriever(): ExperienceRetriever {
  if (!_instance) {
