...
/** 获取当日错误日志文件名 */
function getTodayErrorFilename() {
    const date = new Date().toISOString().split('T')[0];
...
class Logger {
    ...
    maxRotatedFiles = DEFAULT_MAX_ROTATED;
    writeCount = 0;
    rotationCheckInterval = 50;
    _firstWrite = true;
    _rotating = false; // 轮转互斥锁
    _logRetentionDays = DEFAULT_LOG_RETENTION_DAYS;
    // 模块级日志级别
    _moduleLevels = new Map();
    // JSON 格式 + traceId
    _useJsonFormat = false;
    _traceId = null;
    // 日志缓冲区（减少磁盘 I/O）
    _buffer = [];
    _bufferSize = DEFAULT_BUFFER_SIZE;
    _flushIntervalMs = DEFAULT_FLUSH_INTERVAL_MS;
    _flushTimer = null;
    _bufferEnabled = true;
    // WARN 去重：记录每条 WARN 消息关键 key 的最后写入时间
    _lastWarnTimestamps = new Map();
    constructor(logDir) {
        this.logDir = logDir;
     ...
     */
    performRotation(logFile, isErrorLog) {
        this._rotating = true;
        ...
        fs.writeFileSync(logFile, `[${timestamp}] [INFO] [Logger] ${label}已轮转，旧内容已压缩至 ${path.basename(baseName)}.1.gz (${rotatedSizeMB} MB)\n`);
    ...
    checkRotation() {
        ...
        this.writeCount++;
        if (this.writeCount < this.rotationCheckInterval)
            return;
        if (this.writeCount % this.rotationCheckInterval !== 0)
            return;
        const logFile = this.getLogFilePath();
        if (!fs.existsSync(logFile))
            return;
    ...
    writeErrorToFile(timestamp, level, content, moduleName = null) {
        const errFile = this.getErrorLogFilePath();
    ...
    _formatJson(level, module, content, traceId) {
        const obj = {
...
function extractWarnKey(message) {
...
exports.logger = new Logger(logDir);
exports.default = exports.logger;
/**
 * 便捷方法：记录错误并附带上下文
 * 用法：logError('Agent', 'sendMessage 失败', err, { intent: intent.type, msgLen: message.length })
 */
function logError(context, message, error, contextObj) {
    const errMsg = error instanceof Error ? `${error.name}: ${error.message}` : String(error);
    const ctx = contextObj ? ` | ctx: ${JSON.stringify(contextObj)}` : '';
    exports.logger.error(`[${context}] ${message}: ${errMsg}${ctx}`);
    if (error instanceof Error && error.stack) {
        exports.logger.debug(`[${context}] Stack: ${error.stack.split('\n').slice(0, 5).join('\n')}`);
    }
}
/**
 * 便捷方法：记录警告并附带上下文
 */
function logWarn(context, message, contextObj) {
    const ctx = contextObj ? ` | ctx: ${JSON.stringify(contextObj)}` : '';
    exports.logger.warn(`[${context}] ${message}${ctx}`);
}
